  ...
  (let* ((u (string unit)))
    (cond ((and (> (length u) 2) 
            ...
            (list (expansion 'da) 
                  (subseq u 2 (length u))))
          ((and (> (length u) 1) 
                (is-siu (read-from-string (subseq u 1 (length u)))))
            (case (read-from-string (subseq u 0 1))
              ('k (list (expansion 'k) (subseq u 1 (length u))))
              ('h (list (expansion 'h) (subseq u 1 (length u))))
              ('d (list (expansion 'd) (subseq u 1 (length u))))
              ('c (list (expansion 'c) (subseq u 1 (length u))))
              ('m (list (expansion 'm) (subseq u 1 (length u))))))
          (t nil)))))
...
(defun qexpt (quantity exponent)
     (cond
        ((minusp(expt (second quantity) exponent)) 
          (format t "~%Non valgono unità sotto lo zero")))
